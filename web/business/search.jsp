...
<body>
    ...
    <div  class="sw-reg"></div>
		...
		<div class="content fl">
			...
			<ul id="tb_list">
				<c:if test="${empty tenderLst}">
					<li>
						<div class="box">暂无相关条件招标信息！</div>
						<div class="box"></div>
					</li>
				</c:if>
				<c:forEach var="item" items="${tenderLst}">
					<li>
						<div class="box">
							<a href="../business!show.action?id=${item.id}"
								class="${empty sessionScope.login? '  needLogin':''}
              ...
              ${1 eq item.overdue? '  overdue':''}
               ">${item.name}</a><a
								sid="${item.id}" class=" fr tb_btn showTb">[我要投标]</a>
						</div>
						<div class="box">
							<span>招商类型:${item.tenderType}</span> <span>发布时间:${item.pbTime}</span>
							<span>有效时间:${1 eq item.overdue? '已结束':item.endTime}</span> <a
								href="../business!show.action?id=${item.id}"
								class="tb-view  ${empty sessionScope.login? '  needLogin':''}
               ">[查看详细]</a>
						</div>
					</li>
				</c:forEach>
			...
			<div class="info-page" style="padding-left:18px;">
				<pg:pager url="business!search.action"
					items="${businessPage.totalRecords}" maxPageItems="10"
		...
		<div id="zb-list" class=" sidebar fr">
			...
			<ul>
				<c:forEach var="litem" items="${sucessedLst}">
					<li>
						<h3>${litem.name}</h3> <span>${litem.winnerName}</span>
					</li>
				</c:forEach>
			</ul>
		</div>
